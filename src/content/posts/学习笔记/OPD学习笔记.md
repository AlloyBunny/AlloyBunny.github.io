...
最大的问题是**训推不一致**。SFT和蒸馏都有这个问题，他们训练数据的前缀$(x,y_{<t})$中的$y_{<t}$来自外部或者teacher模型。但推理时的$y_{<t}$将来自student模型，student推理时容易遇到训练时没见过的情况，这个问题叫做**误差积累(compounding error)**。

# OPD (On-policy distillation，在线蒸馏)

OPD的loss是：$\mathcal{L}_{OPD}=\mathbb{E}_{x\sim\mathcal{D},y\sim\pi_\theta}\left[\sum_tD_{KL}\left(\pi_T(\cdot|x,y_{<t})\|\pi_\theta(\cdot|x,y_{<t})\right)\right]$

它和蒸馏的loss唯一的区别是，$y$来自$\pi_\theta$而非$\mathcal{D}$。
